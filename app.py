...
if uploaded_file:
    ...
    if partner_query:
        ...
        if item_details:
            ...
            st.warning("No data found for this partner.")import streamlit as st
import pandas as pd
import datetime
import io
from reportlab.lib import colors
...
# Mappings Plano 2
TAG_MAPPING_P2 = {
    'far-east-mto': 'Far East Made to Order', 'far-east-express-tracker': 'Far East Express',
    'far-east-protein-only': 'Far East Protein Only', 'far-east-shrimp': 'Far East Shrimp Entrée',
    'far-east-side-tracker': 'Far East Side/Extra Protein/Shrimp/Egg Rolls',
    'far-east-egg-roll': 'Far East Side/Extra Protein/Shrimp/Egg Rolls',
    'elevated-table-tracker': 'Elevated Table Entrée', 'elevated-table-protein-tracker': 'Elevated Table Protein Only',
    'modern-greens-tracker': 'Modern Greens', 'soup-tracker': 'Modern Greens Soup',
    'market-bar-tracker': 'Market Bar', 'hopes-cookie-tracker': 'Hopes Cookies',
    'sushi-tracker': 'Far East Sushi', 'oda': 'Oda', 'halal': 'Halal', 'a2b': 'A2B', 'lucis': 'Lucis'
}
...
# --- NUEVA LISTA DE DÍAS (SIN FIN DE SEMANA) ---
DAYS_NAME = ['Thursday', 'Friday', 'Monday', 'Tuesday', 'Wednesday']
...
def get_week_range(report_dt):
    # Lógica ajustada para ciclo Jueves-Miércoles
    offset = (report_dt.weekday() - 3) % 7
    start_week = report_dt - datetime.timedelta(days=offset)
    end_week = start_week + datetime.timedelta(days=6)
    return start_week, end_week
...
if uploaded_file:
    ...
    df_sales = pd.read_csv(uploaded_file, skiprows=6).fillna('')
    df_sales['Count'] = df_sales['Count'].apply(lambda x: int(str(x).replace(',', '').strip()) if str(x).replace(',', '').strip().isdigit() else 0)
    ...
    if partner_query:
        ...
        if item_details:
            ...
            prefijo = partner_query.replace('-', ' ').split()[0].upper()
            invoice_num = f"{prefijo}{end_w.strftime('%d%m%y')}"
            
            rows_combined = []
            # Lógica de construcción de filas solo para los 5 días laborales
            for day_name in DAYS_NAME:
                # Calcular la fecha correspondiente al nombre del día dentro de la semana
                day_map = {'Thursday': 0, 'Friday': 1, 'Monday': 4, 'Tuesday': 5, 'Wednesday': 6}
                current_date = start_w + datetime.timedelta(days=day_map[day_name])
                
                is_report_day = current_date.date() == report_dt.date()
                daily_val = partner_sales if is_report_day else 0.0
                
                # Obtener items de forma secuencial para las primeras filas
                idx = DAYS_NAME.index(day_name)
                item_n = df_items.iloc[idx]['Item'] if idx < len(df_items) else ""
                item_c = df_items.iloc[idx]['Count'] if idx < len(df_items) else ""
            ...
            # Items adicionales si hay más de 5
            if len(df_items) > 5:
                for i in range(5, len(df_items)):
                    rows_combined.append({'Date': '', 'Day': '', 'Main St': '', 'Total Net Sales': '', 'Item Detail': df_items.iloc[i]['Item'], 'Total Count': df_items.iloc[i]['Count']})
            ...
            m2.metric("Tax (8.25%)", f"${tax_val:,.2f}")
            m3.metric(f"Partner ({partner_rate*100:.0f}%)", f"${partner_com:,.2f}")
            m4.metric(f"Aramark ({aramark_rate*100:.0f}%)", f"${partner_sales * aramark_rate:,.2f}")
            ...
            metrics_dict = {
                ...
                "Tax (8.25%)": f"${tax_val:,.2f}",
                f"Partner Share ({partner_rate*100:.0f}%)": f"${partner_com:,.2f}",
                f"Aramark Share ({aramark_rate*100:.0f}%)": f"${partner_sales * aramark_rate:,.2f}",
                pay_label: f"${total_pay_val:,.2f}"
            }
